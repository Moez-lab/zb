...
import type { Artwork, About } from '@/types';
...
export async function getArtworks(options?: {
  ...
  try {
    ...
    if (artworks && artworks.length > 0) return artworks as unknown as Artwork[];
  } catch (err) {
    console.warn('[DB Error] Returning fallback artworks:', (err as Error).message);
  }

  // Fallback to defaults
  let filtered = [...DEFAULT_ARTWORKS];
  if (options?.featuredOnly) filtered = filtered.filter((a) => a.featured);
...
export async function getArtworkById(id: string): Promise<Artwork | null> {
  try {
    ...
    if (artwork) return artwork as unknown as Artwork;
  } catch (err) {
    console.warn('[DB Error] Returning fallback for ID:', id);
  }

  return DEFAULT_ARTWORKS.find((a) => a.id === id) || null;
}

export async function getAbout(): Promise<About> {
  try {
    const about = await prisma.about.findUnique({ where: { id: 'singleton' } });
    if (about) return about as unknown as About;
  } catch (err) {
    console.warn('[DB Error] Returning fallback About data');
  }

  return DEFAULT_ABOUT;
}

export async function getTotalAvailableArtworksCount(): Promise<number> {
  try {
    const count = await prisma.artwork.count({ where: { available: true } });
    return count;
  } catch {
    return DEFAULT_ARTWORKS.filter((a) => a.available).length;
  }
}
